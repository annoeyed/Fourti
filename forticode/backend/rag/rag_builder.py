...
class RAGBuilder:
    def __init__(self, cwe_zip_path, owasp_json_path, vector_store_path="faiss_unified_index"):
        ...
        # 파일 확장자 확인
        self.cwe_is_json = cwe_zip_path.endswith('.json') if cwe_zip_path else False
        self.owasp_is_json = owasp_json_path.endswith('.json') if owasp_json_path else False
    ...
    def _load_owasp_documents(self):
        ...
        if not self.owasp_json_path:
            print("Warning: OWASP data path not provided")
            return []
    ...
    def _load_cwe_json_documents(self, cwe_data):
        """CWE JSON 데이터에서 문서를 로드합니다"""
        documents = []
        
        # CWE 데이터가 딕셔너리 형태인 경우 (CWE ID를 키로 사용)
        if isinstance(cwe_data, dict):
            for cwe_id, item in cwe_data.items():
                if isinstance(item, dict):
                    ...
                    # 내용 구성
                    content_parts = [f"{cwe_id}: {name}"]
                    if description:
                        content_parts.append(f"Description: {description}")
                    if examples:
                        content_parts.append(f"Examples:\n" + "\n".join(f"- {ex}" for ex in examples))
                    if mitigations:
                        content_parts.append(f"Mitigations:\n" + "\n".join(f"- {mit}" for mit in mitigations))
                    ...
                    if content.strip():
                        doc = Document(
                            ...
                            metadata={
                                ...
                                "title": item.get('title', ''),
                                "source": item.get('source', 'CWE'),
                                "languages": item.get('languages', []),
                                "technologies": item.get('technologies', []),
                                "languages_inferred": item.get('languages_inferred', []),
                                "security_category": item.get('security_category', 'vulnerability_analysis'),
                                "file_path": item.get('file_path', '')
                            }
                        )
                        documents.append(doc)
    ...
    def _load_and_split_data(self):
        ...
        if os.path.exists(self.cwe_zip_path):
            try:
                if self.cwe_is_json:
                    with open(self.cwe_zip_path, 'r', encoding='utf-8') as f:
                        cwe_data = json.load(f)
                    cwe_documents = self._load_cwe_json_documents(cwe_data)
                    all_documents.extend(cwe_documents)
                    print(f"  - {len(cwe_documents)} CWE documents loaded from JSON")
                else:
                ...
                print(f"Warning: CWE data could not be loaded from {self.cwe_zip_path}")
        else:
            print(f"Warning: CWE file not found at {self.cwe_zip_path}")
        ...
        try:
            ...
            print(f"  - {len(owasp_documents)} OWASP documents loaded")
        except Exception as e:
            print(f"Error loading OWASP data: {e}")
            print(f"Warning: OWASP data could not be loaded from {self.owasp_json_path}")
    ...
    def build_and_save_vector_store(self):
        ...
        if not split_documents:
            ...
            # 빈 문서로 벡터 저장소 생성
            empty_doc = Document(
                page_content="No security data available",
                metadata={"source": "empty", "cwe_id": "N/A"}
            )
            split_documents = [empty_doc]
    ...
    def search(self, query: str, k: int = 5):
        ...
        try:
            ...
            # vector_store가 여전히 None인 경우 처리
            if self.vector_store is None:
                print("Warning: Vector store could not be loaded. Returning empty results.")
                return []
            
            # 벡터 저장소가 비어있는지 확인
            if hasattr(self.vector_store, 'docstore') and hasattr(self.vector_store.docstore, '_dict'):
                if not self.vector_store.docstore._dict:
                    print("Warning: Vector store is empty. Returning empty results.")
                    return []
...
if __name__ == "__main__":
    ...
    owasp_json_file_path = os.path.join(
        os.path.dirname(__file__), "..", "api", "data", "owasp_cheatsheets_parsed.json"
    )

    print(f"Looking for CWE file at: {cwe_zip_file_path}")
    print(f"Looking for OWASP file at: {owasp_json_file_path}")

    # 파일 존재 여부 확인
    if not os.path.exists(cwe_zip_file_path):
        print(f"Error: CWE zip file not found at {cwe_zip_file_path}")
        # CWE JSON 파일로 대체 시도
        cwe_json_file_path = os.path.join(
            os.path.dirname(__file__), "..", "api", "data", "cwe_database.json"
        )
        if os.path.exists(cwe_json_file_path):
            print(f"Using CWE JSON file instead: {cwe_json_file_path}")
            cwe_zip_file_path = cwe_json_file_path
        else:
            print("No CWE data files found!")
            exit(1)
    
    if not os.path.exists(owasp_json_file_path):
        print(f"Error: OWASP JSON file not found at {owasp_json_file_path}")
        exit(1)
    
    print("All data files found. Starting RAG builder...")
