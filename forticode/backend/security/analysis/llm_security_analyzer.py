...
class LLMSecurityAnalyzer:
    ...
    def __init__(self, 
        ...
        self.cwe_db = None  # CWE 데이터베이스는 나중에 설정
...
SAST/DAST 도구 없이 의미론적 분석을 수행하세요."""
    ...
    def analyze_code(self, 
                    ...
                    language: str = "python",
                    context: str = "") -> str:
        """LLM이 직접 코드 보안 분석 수행 - 간단한 프롬프트 사용"""
        try:
            # 간단하고 안정적인 분석 프롬프트 생성
            analysis_prompt = self._create_simple_analysis_prompt(code, language, context)
            
            logger.info(f"LLM 분석 시작 - 언어: {language}, 코드 길이: {len(code)}")
    ...
    def _create_simple_analysis_prompt(self, code: str, language: str, context: str) -> str:
        """간단하고 안정적인 보안 분석 프롬프트 생성"""
        prompt = f"""
...
**Important:** Return ONLY a JSON array. If no vulnerabilities found, return empty array [].
